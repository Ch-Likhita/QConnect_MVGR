...
export default function EditProfilePage() {
  ...
  useEffect(() => {
    ...
    if (user.role === 'student' && user.studentProfile) {
      setRollNumber(user.studentProfile.rollNumber || '');
      setBranch(user.studentProfile.branch || 'CSE');
      setCurrentAcademicYear(user.studentProfile.currentAcademicYear || 1);
      setCurrentCGPA(user.studentProfile.currentCGPA);
      setPrimaryCareerGoal(user.studentProfile.primaryCareerGoal || 'Internships');
      setInterestedCareerPathways(user.studentProfile.interestedCareerPathways || []);
      setSkills(user.studentProfile.skills || []);
      setPlacementStatus(user.studentProfile.placementStatus);
      setCompanyName(user.studentProfile.companyName || '');
    } else if (user.role === 'alumni' && user.alumniProfile) {
      setGraduationYear(user.alumniProfile.graduationYear || 2020);
      setDegree(user.alumniProfile.degree || 'B.Tech');
      setAlumniBranch(user.alumniProfile.branch || '');
      setCurrentCompany(user.alumniProfile.currentCompany || '');
      setCurrentRole(user.alumniProfile.currentRole || '');
      setYearsOfExperience(user.alumniProfile.yearsOfExperience || 0);
      setCareerDomain(user.alumniProfile.careerDomain || 'Software');
      setAlumniSkills(user.alumniProfile.skills || []);
      setCareerPathwayTags(user.alumniProfile.careerPathwayTags || []);
    } else if (user.role === 'faculty' && user.facultyProfile) {
      setFacultyDepartment(user.facultyProfile.department || '');
      setDesignation(user.facultyProfile.designation || 'Assistant Professor');
      setFacultyYearsOfExperience(user.facultyProfile.yearsOfExperience || 0);
      setSubjectsTaught(user.facultyProfile.subjectsTaught || []);
      setGuidanceAreas(user.facultyProfile.guidanceAreas || []);
    } else if (user.role === 'recruiter' && user.recruiterProfile) {
      setOfficialEmail(user.recruiterProfile.officialEmail || '');
      setCompanyNameRecruiter(user.recruiterProfile.companyName || '');
      setCompanyType(user.recruiterProfile.companyType || 'Service');
      setRolesHiringFor(user.recruiterProfile.rolesHiringFor || []);
      setEligibleBranches(user.recruiterProfile.eligibleBranches || []);
    }
  }, [loading, user, router]);
    ...
    try {
      ...
      if (user.role === 'student') {
        ...
        updateData.recruiterProfile = {
          ...
          rolesHiringFor,
          eligibleBranches,
          canViewAlumniProfiles: user.recruiterProfile?.canViewAlumniProfiles || false,
          canPostOpportunities: user.recruiterProfile?.canPostOpportunities || false,
        };
      }
